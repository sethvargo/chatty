...
if [ -z $NOTAG ]; then
  echo "==> Tagging..."
  git commit --allow-empty -a --gpg-sign=F86214C0 -m "Release v$VERSION"
  git tag -a -m "Version $VERSION" -s -u F86214C0 "v${VERSION}" master
fi
...
if [ -z $NOSIGN ]; then
  echo "==> Signing..."
  gpg --default-key F86214C0 --detach-sig ./${NAME}_${VERSION}_SHA256SUMS
fi
popd
